...
func check(g common.Grid, row int, col int, taken []common.Pos) (int, []common.Pos) {
	...
	if StringsToIntwithO(g.CheckLeft(row, col)) == curloc+1 {
		/*if curloc == 8 && !CheckIfPosTaken(taken, row, col-1) {
			PrintEnd(g, row, col)
			res += 1
			taken = append(taken, common.Pos{row, col - 1})
			return res, taken
		} elseif CheckIfPosTaken(taken, row, col-1) {
			return res, taken
		} else {
		*/
		tres := 0
	...
	if StringsToIntwithO(g.CheckUp(row, col)) == curloc+1 {
		/*if curloc == 8 && !CheckIfPosTaken(taken, row-1, col-1) {
			PrintEnd(g, row, col)
			res += 1
			taken = append(taken, common.Pos{row - 1, col})
			return res, taken
		} else if CheckIfPosTaken(taken, row, col-1) {
			return res, taken
		} else { */
		tres := 0
	...
	if StringsToIntwithO(g.CheckDown(row, col)) == curloc+1 {
		/*if curloc == 8 && !CheckIfPosTaken(taken, row+1, col) {
			PrintEnd(g, row, col)
			res += 1
			taken = append(taken, common.Pos{row + 1, col})
			return res, taken
